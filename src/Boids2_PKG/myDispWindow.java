...
public abstract class myDispWindow {
	...
	public static final float xOff = 20 , yOff = 20.0f * (Boids_2.txtSz/12.0f), btnLblYOff = 2 * yOff, rowStYOff = yOff*.15f;
	...
	public static final int[] editCrcFillClrs = new int[] {Boids_2.gui_FaintMagenta, Boids_2.gui_FaintGreen};			
	...
	public myDispWindow(Boids_2 _p, String _n, int _flagIdx, int[] fc,  int[] sc, float[] rd, float[] rdClosed, String _winTxt, boolean _canDrawTraj) {
		pa=_p;
		ID = winCnt++;
		name = _n;
		pFlagIdx = _flagIdx;
		...
		winText = _winTxt;
		trajFillClrCnst = Boids_2.gui_Black;		//override this in the ctor of the instancing window class
		trajStrkClrCnst = Boids_2.gui_Black;
...
//		tmp = new myGUIObj(pa, this,i, guiObjName, xyDims[0], xyDims[1], xyDims[2], xyDims[3], guiMinMaxModVals, guiStVal, guiBoolVals, off);		
	...
	protected void showKeyPt(myPoint a, String s, float rad){	pa.show(a,rad, s, new myVector(10,-5,0), Boids_2.gui_Cyan, getFlags(trajPointsAreFlat));	}	
	...
	protected void dispBttnAtLoc(String txt, float[] loc, int[] clrAra){
		pa.setFill(clrAra);
		pa.setColorValStroke(Boids_2.gui_Black);
		pa.rect(loc);		
		pa.setColorValFill(Boids_2.gui_Black);
		//pa.translate(-xOff*.5f,-yOff*.5f);
	...
	protected void drawMouseBox(){
		if( getFlags(showIDX)){
		    pa.setColorValFill(Boids_2.gui_LightGreen );
			pa.rect(closeBox);
			pa.setFill(strkClr);
			pa.text("Close" , closeBox[0]-35, closeBox[1]+10);
		} else {
		    pa.setColorValFill(Boids_2.gui_DarkRed);
			pa.rect(closeBox);
			pa.setFill(strkClr);
...
//		pa.popStyle();pa.popMatrix();		
	...
	protected void drawNotifications(){		
		...
		dispMenuTxtLat("Drawing curve", pa.getClr((getFlags(drawingTraj) ? Boids_2.gui_Green : Boids_2.gui_Red)), true);
		//pa.show(new myPoint(0,0,0),4, "Drawing curve",new myVector(10,15,0),(getFlags(this.drawingTraj) ? pa.gui_Green : pa.gui_Red));
		//pa.translate(0,-30);
		dispMenuTxtLat("Editing curve", pa.getClr((getFlags(editingTraj) ? Boids_2.gui_Green : Boids_2.gui_Red)), true);
...
//	public static final String[] guiBtnSetValsNames = new String[]{"Set Vals at Cur Time","Set Vals Globally"};
	...
	public mySideBarMenu(Boids_2 _p, String _n, int _flagIdx, int[] fc, int[] sc, float[] rd, float[] rdClosed, String _winTxt, boolean _canDrawTraj) {
		super(_p, _n, _flagIdx, fc, sc,  rd, rdClosed, _winTxt, _canDrawTraj);
		guiBtnStFillClr = new int[]{		//button colors based on state
				Boids_2.gui_White,								//disabled color for buttons
				Boids_2.gui_LightGray,								//not clicked button color
				Boids_2.gui_LightBlue,									//clicked button color
			};
		guiBtnStTxtClr = new int[]{			//text color for buttons
				Boids_2.gui_LightGray,									//disabled color for buttons
				Boids_2.gui_Black,									//not clicked button color
				Boids_2.gui_Black,									//clicked button color
			};			
...
class myDrawnSmplTraj {
	...
	public myDrawnSmplTraj(Boids_2 _p, myDispWindow _win,float _topOffy, int _fillClrCnst, int _strkClrCnst, boolean _flat, boolean _smCntl){
		pa = _p;
...
class myScrollBars{
	...
	public myScrollBars(Boids_2 _pa,myDispWindow _win){
		pa = _pa;
		win = _win;
		ID = scrBarCnt++;
		setSize();
