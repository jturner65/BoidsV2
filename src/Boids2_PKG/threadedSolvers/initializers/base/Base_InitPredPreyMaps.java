...
public abstract class Base_InitPredPreyMaps implements Callable<Boolean> {
     ...
     */
    private float _calcMinSqDist1D(float p1, float p2, float dim, float[] newP1, float[] newP2){
        float p1Mdim = p1 - dim;
        float p2Mdim = p2 - dim;
        float d1 = (p1-p2),      d1s = d1*d1,
            d2 = (p1-p2Mdim),    d2s = d2*d2,
            d3 = (p1Mdim-p2),    d3s = d3*d3;
        if(d1s <= d2s){
            if(d1s <= d3s){   newP1[0] = p1;    newP2[0] = p2;      return d1s;}     //d1s is min, for this dim newP1 == p1 and newP2 == p2
            else {            newP1[0] = p1Mdim;newP2[0] = p2+dim;  return d3s;}}    //d3s is the min
        else {
            if(d2s <= d3s){   newP1[0] = p1+dim;newP2[0] = p2Mdim;  return d2s;}     //d2s is min, for this dim newP1 == p1 and newP2 == p2 
            else {            newP1[0] = p1Mdim;newP2[0] = p2+dim;  return d3s;}}    //d3s is the min
    }// calcMinSqDist1D
...
//        if(sqDist <= min2Dist){return sqDist;}            
        ...
        float dx = _calcMinSqDist1D(pt1.x, pt2.x, dimX, newP1, newP2);
        newPt1.x = newP1[0];newPt2.x = newP2[0];
        float dy = _calcMinSqDist1D(pt1.y, pt2.y, dimY, newP1, newP2);
        newPt1.y = newP1[0];newPt2.y = newP2[0];
        float dz = _calcMinSqDist1D(pt1.z, pt2.z, dimZ, newP1, newP2);
